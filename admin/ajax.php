...
switch ($_QET['act']) {
    ...
    case 'setClassSort': //修改分类排序
        $cid = (int)$_QET['cid'];
        $type = (int)$_QET['type'];
        if (\Admin\Admin::ClassSort($cid, $type)) {
    ...
    case 'ChangeOrders': //修改订单状态
        ...
        $DB = SQL::DB();
        $re = $DB->get('order', ['uid', 'payment', 'order', 'state'], [
            'id' => (int)$_QET['did'],
        ]);
        if ($re['state'] == $_QET['id']) {
            dies(-1, '订单状态未改变，调整失败！');
    ...
    case 'login_account': //账号密码登陆
        ...
        if (empty($_SESSION['Login_uvc']) || $_SESSION['Login_uvc'] <> md5($_QET['vercode'] . href()) && $conf['VerificationCodeSwitch'] != 2) dies(-2, '验证码有误！');
    ...
    case 'Send_verification_code_login': //短信登陆
        ...
        if (empty($_SESSION['AdminLogin_sms_vis']) || $_SESSION['AdminLogin_sms_vis'] != md5($_QET['code'] . href()) && $conf['VerificationCodeSwitch'] != 2) {
            dies(-2, '验证码有误！');
        }

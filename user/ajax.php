...
switch ($_QET['act']) {
    case 'user_set': //修改用户信息
        ...
        if ($Res) {
            Hook::execute('UserModifyInform', [
                'phone' => $_QET['mobile'],
                'qq' => $_QET['qq'],
                'username' => $_QET['username'],
                'password' => $_QET['password'],
                'name' => $_QET['name'],
                'uid' => $UserData['id'],
            ]);
    ...
    case 'login_account': //账号密码登陆
        ...
        if (empty($_SESSION['Login_vc']) || $_SESSION['Login_vc'] <> md5($_QET['vercode'] . href()) && $conf['VerificationCodeSwitch'] != 2) {
        ...
        userlog('后台登陆', '用户于' . $date . '通过账号密码成功登陆后台！', $Res['id'], '0');
    ...
    case 'login_register': //用户注册！
        ...
        if (empty($_SESSION['Login_res']) || $_SESSION['Login_res'] <> md5($_QET['vercode'] . href()) && $conf['VerificationCodeSwitch'] != 2) {
    ...
    case 'user_pay':
        ...
        if (empty($_QET['money']) || empty($_QET['type'])) dies(-1, '请提交完整哦!');
    ...
    case 'Send_verification':
        ...
        if (empty($_QET['code'])) dies(-1, '请填写完整！');
    ...
    case 'configuration_save':
        ...
        if ($UserData['configuration'] == '') {
            ...
            $configuration_arr = json_decode($UserData['configuration'], true);
        }
        ...
        switch ($_QET['type']) {
            case 'domain':
                ...
                break;
            case 'template':
                if ($UserData['grade'] < $conf['usergradetem']) dies(-2, '您当前等级无法装修店铺,请先去提升等级吧！');
                ...
                $configuration_arr = json_encode($configuration_arr);
                break;
            case 'webset':
                if ($UserData['grade'] < $conf['usergradenotice']) dies(-2, '您当前等级无法配置店铺基础信息,请先去提升等级吧！');
                $configuration_arr = array_merge($configuration_arr, [
                    ...
                    'FluctuationsPrices' => $_QET['FluctuationsPrices'],
                    'LevelDisplay' => $_QET['LevelDisplay'],
                    'RecordNumber' => $_QET['RecordNumber'],
                    'UserBackgroundTips' => $_QET['UserBackgroundTips'],
                    'kfwx' => $_QET['kfwx'],
                    'logo' => $_QET['logo'],
                    'appurl' => $_QET['appurl'],
                    'statistics' => $_QET['statistics'],
                    ...
                    'ServiceImageQQ' => $_QET['ServiceImageQQ'],
                ...
                break;
            case 'notice':
                if ($UserData['grade'] < $conf['usergradenotice']) dies(-2, '您当前等级无法配置店铺公告哦,请先去提升等级吧！');
                $configuration_arr = array_merge($configuration_arr, [
                    'notice_top' => base64_encode($_REQUEST['notice_top']),
                    'notice_check' => base64_encode($_REQUEST['notice_check']),
                    'notice_bottom' => base64_encode($_REQUEST['notice_bottom']),
                    'PopupNotice' => base64_encode($_REQUEST['PopupNotice']),
                ]);
                $configuration_arr = json_encode($configuration_arr);
                break;
            default:
        ...
        $i = json_decode($configuration_arr, true);
        if (!$i || count($i) < 3) {
    ...
    case 'WithdrawDeposit':
        ...
        if ($UserData['grade'] < $conf['userdepositgrade']) dies(-2, '您当前等级无法使用提现功能,请先去提升等级吧！');
    ...
    case 'DoGatheringFigure':
        ...
        if ($UserData['grade'] < $conf['userdepositgrade']) dies(-2, '您当前等级无法使用提现功能,请先去提升等级吧！');
        $images = PictureUpload::uploadCashWithdrawChart();
        dier([
            'code' => 0,
            'msg' => '上传成功,上传的收款码大小为：' . $_QET['file']['size'] / 1000 . 'kb',
            'src' => ImageUrl($images) . '?time=' . time()
        ]);
        break;
    case 'withdraw_deposit':
        ...
        $page = ((int)$_QET['page'] - 1) * (int)$_QET['limit'];
    ...
    case 'Tickets': //工单管理
        ...
        switch ($_QET['type']) {
            case 'list':
                $sql = ['uid' => $UserData['id']];
                if ($_QET['state'] === 'all') {
                    $List = $DB->select('tickets', ['id', 'order', 'name', 'class', 'state', 'addtime', 'grade', 'type'], ['uid' => $UserData['id'], 'ORDER' => ['id' => 'DESC'], 'LIMIT' => [$page, $limit]]);
                } else {
                    $sql['state'] = $_QET['state'];
                    $List = $DB->select('tickets', ['id', 'order', 'name', 'class', 'state', 'addtime', 'grade', 'type'], ['uid' => $UserData['id'], 'state' => $_QET['state'], 'ORDER' => ['id' => 'DESC'], 'LIMIT' => [$page, $limit]]);
                }
                dier([
                    'code' => 1,
                    'msg' => '工单列表获取成功',
                    'data' => $List,
                    'count' => [
                        'a1' => $DB->count('tickets', ['uid' => $UserData['id']]), //全部工单
                        'a2' => $DB->count('tickets', ['uid' => $UserData['id'], 'state' => 1]), //已解决
                        'a3' => $DB->count('tickets', ['uid' => $UserData['id'], 'state' => 2]), //处理中
                        'a4' => $DB->count('tickets', ['uid' => $UserData['id'], 'state' => 3]), //已关闭
                    ],
                    "num" => $DB->count('tickets', $sql) - 0,
                    'url' => href(2),
                ]);
                break;
            case 'details': //工单详情
    ...
    case 'AlterPrice': //商品价格调整！
        if (!$UserData) dies(-1, '请先完成登陆！');
        if ($UserData['grade'] < $conf['usergradeprofit']) dies(-2, '您当前等级无法设置商品价格,请先去提升等级吧！');
    ...
    case 'GoodsState': //调整商品上下架状态
        if (!$UserData) dies(-1, '请先完成登陆！');
        if ($UserData['grade'] < $conf['usergradegoodsstate']) dies(-2, '您当前等级无法设置商品上下架状态,请先去提升等级吧！');
    ...
    case 'GoodsList': //获取商品列表
        ...
        $Deploy = json_decode($UserData['pricehike'], TRUE); //商品配置
        foreach ($Res as $res) {
            $price = Price::Get($res['money'], $res['profits'], $UserData['grade'], $res['gid'], $res['selling'], $res['profit_rule']);
            ...
            if (isset($Deploy[$res['gid']]['rise']) && !empty($Deploy[$res['gid']]['rise'])) {
                $Rise = (max((float)$Deploy[$res['gid']]['rise'], 0));
            } else {
                $Rise = 0;
            }
            ...
            $Data[] = [
                ...
                'docs' => $res['docs'],
                'sales' => $res['sales'],
                'state' => ($Deploy[$res['gid']]['state'] ?? 1), //商品自定义状态
    ...
    case 'CouponList': //获取优惠券列表
        ...
        foreach ($Res as $v) {
            ...
            $v['expirytime'] = (($TIME - time()) <= 0 ? 0 : ($TIME - time()));
            $Data[] = $v;
        }
    ...
    case 'AppAdd': //创建App生成任务
        if (!$UserData) dies(-1, '请先完成登陆！');
        if ($conf['userappstate'] != 1) {
            dies(-1, '当前站点已经关闭了APP生成功能！');
        }
    ...
    case 'AppColorSet': //修改App任务配色
        ...
        if ($Res) {
            dies(1, ($_QET['type'] == 1 ? '导航栏颜色' : '加载条颜色') . '修改成功！');
        } else dies(-1, '修改失败！');
        break;
    case 'AppSet':
        if (!$UserData) dies(-1, '请先完成登陆！');
    ...
    case 'HierarchicalData': //等级页面数据
        ...
        foreach ($RankList as $key => $val) {
            ...
            //计算时间差价
            if ($Price > 0 && !empty($UserData['endtime'])) {
                if ($UserData['endtime'] > $date) {
                    $Time = ((strtotime($UserData['endtime']) - time()) / 86400) - ($conf['PermissionLengthTime'] - 0);
                    if ($Time > 0) {
                        $UpMoney = round(((round($val['money'], 8) * ($conf['PermissionRenewPrice'] / 100)) / ($conf['PermissionLengthTime'] - 0)) * $Time, 2);
                        $Price += $UpMoney;
                        $Price = round($Price, 2);
                        $val['content'] .= '<br><br>Ps：其中<font color="red">' . round($Time) . '天</font>会员有效期已换算为<font color="red">' . $UpMoney . '元</font>，升级后<font color="#fa8072">自动继承</font>权限有效期(' . round(($Time + $conf['PermissionLengthTime']), 0) . '天)！<br>
<br>继承天数计算公式：' . round(($Time + $conf['PermissionLengthTime']), 0) . '<font color="red">`剩余到期天数`</font>-' . $conf['PermissionLengthTime'] . '<font color="red">`开通赠送天数`</font>=' . round($Time) . '<font color="red">`需要继承的天数`</font>
<br>
<br>升级金额计算公式如下(部分参数四舍五入)：<br>((' . round($val['money'], 2) . '<font color="red">`当前等级原价`</font> × ' . $conf['PermissionRenewPrice'] . '%<font color="red">`续费金额百分比`</font>) ÷ ' . ($conf['PermissionLengthTime'] - 0) . '<font color="red">`每次续期的天数`</font>) × ' . round($Time) . '<font color="red">`剩余到期天数`</font>) + (' . round($val['money']) . '<font color="red">`目标等级原价`</font>-' . round($Giv['money']) . '<font color="red">`当前等级原价`</font>)) = ' . $Price . '<font color="red">`最终权限升级价格`</font>';
                    }
                }
            }
        ...
        if (empty($Giv)) {
            ...
            if ($Giv['sort'] >= count($ListRank)) {
                $Giv['sort'] = count($ListRank);
            }
            $Giv = [
                'name' => $Giv['name'],
                'mid' => $Giv['sort'],
    ...
    case 'LevelUp': //等级提升
        ...
        //开启了权限有效期
        if ((int)$conf['PermissionTime'] == 1 && ($update + 1) >= (int)$conf['userleaguegrade']) {
            //启用有效期
            if (empty($UserData['endtime']) || $UserData['endtime'] <= $date) {
                $Day = $conf['PermissionLengthTime'];
                $DataEnd = date("Y-m-d H:i:s", strtotime(" + $Day day"));
                $SQL['endtime'] = $DataEnd;
            }
        }
        $UpRes = $DB->update('user', $SQL, [
            'id' => $UserData['id']
        ]);
        if ($UpRes) {
            UserConf::Commission($UserData['id'], $Data['OrPrice'], $UserData['superior'], $update + 1);
            if ($Data['UpPrice'] != $Data['OrPrice']) {
                userlog('等级提升', '您在后台消耗余额将用户等级提升为' . $Data['name'] . ',消耗金额为：' . $Data['UpPrice'] . '元！，其中' . ($Data['UpPrice'] - $Data['OrPrice']) . '元是时长续期费用！', $UserData['id'], $Data['UpPrice']);
            } else {
    ...
    case 'PayData': //钱包数据
        ...
        dier([
            ...
            'Conf' => [ //充值配置
                ...
                'RsPay' => explode(',', $conf['userdeposittype']), //用户提现支持方式
                'PayLog' => [
                ...
                'ChargeMinimum' => (explode('-', $conf['RechargeLimit'])[0] ?? 0.01) - 0, //最低充值金额
                'currency' => $conf['currency'],
                'PayUrl' => $conf['rechargepurchaseurl'] ?? false, //充值卡购买地址
                'karmaRechargeSwitch' => ($conf['karmaRechargeSwitch'] == 1), //卡密充值功能开关
                'RechargeRange' => $conf['RechargeLimit'], //充值金额区间
            ]
        ]);
        break;
    case 'SignData': //签到数据
        ...
        dier([
            ...
            'data' => [
                ...
                'count' => $Count, //签到总数
                'currency' => $State, //奖励类型
                'number' => $conf['GiftContent'] ?? ($State == '余额' ? '0.01-0.1' : '1-100'), //奖励内容
            ]
        ]);
        break;
    case 'SignIn': //每日签到
    ...
    case 'ShopData': //店铺数据
        ...
        if ($UserData['grade'] < $conf['userleaguegrade']) dies(-2, '您当前的等级无法开通小店,请先去提升等级！，需达到' . $conf['userleaguegrade'] . '级！');
        $DB = SQL::DB();
        ...
        $DataSite = json_decode($UserData['configuration'], true);
        if (!$DataSite) {
            $DataSite = $conf;
        }
    ...
    case 'StoreConf': //店铺配置信息
        ...
        $DataConf = json_decode($UserData['configuration'], true);
        if ($DataConf) {
        ...
        dier([
            ...
            'data' => [
                ...
                'title' => $DataConf['title'], //网站副标题
                'keywords' => $DataConf['keywords'],
                'description' => $DataConf['description'],
                'kfqq' => $DataConf['kfqq'],
                'kfwx' => $DataConf['kfwx'],
                'logo' => $DataConf['logo'],
                'statistics' => $DataConf['statistics'], //客服JS代码
                'RecordNumber' => $DataConf['RecordNumber'], //网站备案号,留空隐藏
                'UserBackgroundTips' => $DataConf['UserBackgroundTips'], //用户后台小提示,留空隐藏
                'ServiceTips' => $DataConf['ServiceTips'], //客服界面提示信息
                'CartState' => $DataConf['CartState'], //购物车开关
                'ServiceImage' => $DataConf['ServiceImage'], //微信客服二维码
                'ServiceImageQQ' => $DataConf['ServiceImageQQ'], //QQ客服二维码
                'Communication' => $DataConf['Communication'], //官方群链接
                'currency' => $DataConf['currency'], //货币名称
                'appurl' => $DataConf['appurl'],
                'ForcedLanding' => $DataConf['ForcedLanding'], //1不强制用户登陆,2强制用户必须登陆,3看到商品后才提示登陆！
                'DynamicMessage' => $DataConf['DynamicMessage'], //站点动态消息开关
                'GoodsRecommendation' => $DataConf['GoodsRecommendation'], //开启或商品推荐[部分模板生效]
                'SimilarRecommend' => $DataConf['SimilarRecommend'], //是否开启同分类商品推荐
                'FluctuationsPrices' => $DataConf['FluctuationsPrices'], //价格波动展示开关
                'LevelDisplay' => $DataConf['LevelDisplay'], //等级价格列表展示
            ],
        ]);
        break;
    ...
    case 'InviteData': //邀请详情
        ...
        dier([
            ...
            'data' => [
                ...
                'a1' => $count_1, //总邀请人数
                'a2' => $count_2, //总领取次数
                'award' => $conf['award'], //邀请奖励内容
                'image' => href(2) . ROOT_DIR . 'user/image/api.php?url=' . base64_encode($Url) . '&type=' . $Type . '&ids=' . $rand . '&images=uid' . $UserData['id'] . '_' . $rand . '.jpg', //邀请图
                'List' => reward::Invite_statistics($UserData, $_QET['page'], $_QET['limit']), //邀请列表
                'count' => $count_1,//总邀请人数
                'currency' => $conf['currency'] //获取名称
            ],
        ]);
        break;
    ...
    case 'UserGet': //用户信息
        ...
        dier([
            ...
            'data' => [
                ...
                'id' => $UserData['id'],
                'token' => $token,
                'IPList' => (empty($UserData['ip_white_list']) ? [] : explode('|', $UserData['ip_white_list'])),
                'url' => href(2),
            ]
        ]);
        break;
    case 'AppMenuList': //读取应用菜单栏信息
        dier([
            'code' => 1,
            'msg' => '数据获取成功',
            'data' => AppList::AppMenuList("MenuUser")
        ]);
        break;
    case 'app_view': //打开视图文件！
        ...
        break;
    case 'PermissionRenewal': //续期权限
        if (!$UserData) {
            dies(-1, '请先完成登陆！');
        }
        if ($conf['PermissionTime'] == 2) {
            dies(-1, "未开放权限续期模式，无需续期！");
        }
        if (empty($UserData['endtime'])) {
            dies(-1, "您当前权限永久有效，无需续期！");
        }
        $Umid = RatingParameters($UserData);
        $Price = ($Umid['money'] ?? 100) * ($conf['PermissionRenewPrice'] / 100);
        $Price = round($Price, 2);
        if ($UserData['money'] < $Price) {
            dies(-2, '当前余额不足' . $Price . '元，无法续期，请先充值！');
        }
        $Day = $conf['PermissionLengthTime'];
        if ($UserData['endtime'] > $date) {
            $DataEnd = date("Y-m-d H:i:s", strtotime($UserData['endtime'] . " + $Day day"));
        } else {
            $DataEnd = date("Y-m-d H:i:s", strtotime(" + $Day day"));
        }
        $DB = SQL::DB();
        ...
        if ($Res) {
            userlog('权限续期', '您于' . $date . '花费了' . $Price . '元将您的权限到期时间续期到了' . $DataEnd, $UserData['id'], $Price);
            Hook::execute('RenewUserRight', [
                'uid' => $UserData['id'],
                'money' => $Price,
                'endtime' => $DataEnd
            ]);
            dies(1, '您成功花费' . $Price . '元将您的权限到期时间续期到了' . $DataEnd);
        } else {
            dies(-1, '续期失败，请重新尝试！');
        }
        break;
    case 'notificationGet': //查看公告详情
        if (!$UserData) {
            dies(-1, '请先完成登陆！');
        }
        test(['id|e']);
        ...
        break;
    case 'rechargeRecord': //在线充值记录
        if (!$UserData) {
            dies(-1, '请先完成登陆！');
        }
        $DB = SQL::DB();
        $List = $DB->select('pay', [
            'type', 'name', 'money', 'price', 'endtime', 'addtime'
        ], [
            'gid' => -1,
            'uid' => 1,
            'state' => 1,
        ...
        break;
    case 'storeOrder': //查询店铺下面的订单[简易列表]
        if (!$UserData) {
            dies(-1, '请先完成登陆！');
        }
        $page = ((int)$_QET['page'] - 1) * (int)$_QET['limit'];
        ...
        $Res = $DB->select('order', [
            ...
            'id', 'order.uid', 'order.num', 'order.gid', 'order.payment', 'order.price', 'addtitm(addtime)', 'finishtime(endtime)', 'order.state',
            'goods.name', 'goods.quantity', 'goods.units', 'originalprice', 'coupon'
        ], [
            'muid' => $UserData['id'],
            'ORDER' => [
                'id' => 'DESC'
    ...
    case 'TemData': //获取模板单独配置参数
        if (isset($_QET['name'])) {
            //获取指定
            $TemState = TemConf($_QET['name'], 2, 2);
        } else {
            //获取当前
            $Name = $conf['UserBackgroundTemplatePC'];
            if (View::isMobile() === true) {
                $Name = $conf['UserBackgroundTemplateM'];
            }
            $TemState = TemConf($Name, 2, 2);
        }
        if (!$TemState) {

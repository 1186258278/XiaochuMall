...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?= ROOT_DIR ?>assets/layui/css/layui.css"/>
    <link rel="stylesheet" href="<?= ROOT_DIR ?>assets/easyeditor/css/easyeditor.css">
    <link rel="stylesheet" type="text/css" href="<?= ROOT_DIR ?>assets/easyeditor/css/fonticon.css"/>
</head>
<body>
                    ...
                    <div class="layui-card">
                        <div class="layui-card-header">
                            <img src="<?= UserImage($UserData) ?>" style="width: 20px;height: 20px;"/> <span
                                    style="color:#030707"><?= htmlentities($UserData['name']) ?></span>
...
<script src="<?= ROOT_DIR ?>assets/layuiadmin/layui/layui.js" type="text/javascript" charset="utf-8"></script>
<script src="<?= ROOT_DIR ?>assets/easyeditor/js/marked.min.js" type="text/javascript" charset="utf-8"></script>
...
<script>
    layui.config({
        ...
        easyeditor.init({
            elem: '.editor'
            , uploadUrl: '<?= ROOT_DIR ?>user/ajax.php?act=image_up'
        ...
        form.on('submit(tickets)', function (data) {
            layer.open({
                ...
                btn1: function () {
                    $.ajax({
                        type: "post",
                        url: "<?= ROOT_DIR ?>user/ajax.php?act=Tickets&type=Supplementary",
                        data: data.field,
                        dataType: "json",
                        success: function (data) {
                            if (data.code == 1) {
                                layer.alert(data.msg, {
                                    icon: 1, title: '温馨提示', end: function () {
                                        $("#L_content").val('');
                                        location.reload();
                                    }
                                });
    ...
    function details() {
        ...
        $.ajax({
            type: "post",
            url: "<?= ROOT_DIR ?>user/ajax.php?act=Tickets&type=details&id=" + $("input[name='id']").val(),
            dataType: "json",
            success: function (data) {
                layer.closeAll();
                if (data.code == 1) {
    ...
    function grade() {
        layer.open({
            title: '评分面板',
            content: '<div id="grade"></div><br>您对本次售后工单处理还满意吗?',
            btn: ['提交', '取消'],
            success: function () {
                ...
                $.ajax({
                    type: "post",
                    url: "<?= ROOT_DIR ?>user/ajax.php?act=Tickets&type=Grade&id=" + $("input[name='id']").val() + '&n=' + $("input[name='grade']").val(),
                    dataType: "json",
                    success: function (data) {
                        if (data.code == 1) {
    ...
    function finish() {
        layer.open({
            ...
            btn1: function () {
                $.ajax({
                    type: "post",
                    url: "<?= ROOT_DIR ?>user/ajax.php?act=Tickets&type=Finish&id=" + $("input[name='id']").val(),
                    dataType: "json",
                    success: function (data) {
                        if (data.code == 1) {
                            layer.alert(data.msg, {

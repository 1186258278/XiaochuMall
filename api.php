...
switch ($_QET['act']) {
    ...
    case 'Docking_goods': //获取商品状态/价格等
        ...
        if ((int)$Goods['specification'] === 2) {
            ...
            foreach ($Goods['specification_sku'] as $key => $value) {
                if ($value['money'] !== '') {
                    $GoodsPrice = Price::Get($value['money'], $Goods['profits'], $User['grade'], $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
        ...
        $Price = Price::Get($Goods['money'], $Goods['profits'], $User['grade'], $gid, $Goods['selling'], $Goods['profit_rule']);
        dier([
            'code' => 1,
            'msg' => '数据获取成功',
            'data' => [
    ...
    case 'UserMoneyApi': //用户加款/扣钱
        if (empty($_QET['token']) || empty($_QET['uid']) || empty($_QET['type']) || empty($_QET['money'])) dies(-1, '请填写完整！');
    ...
    case 'OrdersTesting': //漏单监控(目前只支持易支付！)
        if ($conf['secret'] !== $_QET['token'] && $conf['monitor_key'] != $_QET['token']) {
            dies(-1, 'API监控密钥有误！');
        }
    ...
    case 'OrderList': //取出订单列表
        if ($conf['secret'] <> $_QET['token']) dies(-1, 'API对接密钥有误！');
        if (empty($_QET['state'])) dies(-1, '请将需要取出的订单类型填写完整!<br>类型：1成功，2待处理，3异常，4正在处理，5退款,6售后维权,7已评价');
        if (empty($_QET['limit'])) dies(-1, '请将需要取出的数量填写完整!');
        $DB = SQL::DB();
    ...
    case 'OrderSet': //修改订单状态,可修改订单状态,返回内容,货源余额,订单备注
        if ($conf['secret'] != $_QET['token']) dies(-1, 'API对接密钥有误！');
        if (empty($_QET['state'])) dies(-1, '请将订单修改后的状态填写完整:state！<br>类型：1成功，2待处理，3异常，4正在处理，5退款,6售后维权,7已评价');
        if (empty($_QET['remark'])) dies(-1, '请将订单备注填写完整:remark!');
        if (empty($_QET['user_rmb'])) $_QET['user_rmb'] = 0;
    ...
    case 'OrderStatusMonitoring': //订单状态监控
        ...
        $count = $_QET['num'] ?? 60; //每次监控的订单数量
        if ($count > 60 || $count < 1) {
            dies(-1, '单次订单的监控数量参数提交有误,监控数量范围：1 - 60');
        }
        Order::OrderStatusMonitoring($count);
        break;
    ...
    case 'OnebuttonDockingSystem': //同系统对接模块
        ...
        break;
    case 'AdminTransfer': //主站后台api转接接口
        test(['Types|e', 'Pattern|e', 'DockingKey|e']);
        if ($conf['secret'] != $_QET['DockingKey']) {
            dies(-1, 'API对接密钥有误！');
        }
        $_QET['act'] = $_QET['Pattern'];
        unset($_QET['DockingKey']);
        $_AdminApi = true;
        if ($_QET['Types'] == 1) {
            unset($_QET['Types'], $_QET['Pattern']);

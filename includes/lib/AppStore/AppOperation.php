...
class AppList
{
    ...
    {
        $Rule = Rule;
        if (count($Rule) === 0 || empty($Input)) {
            return false;
        }
        foreach ($Rule as $key => $value) {
            if ($value['type'] == 3 && strstr($Input, '[Multiple]')) {
                $re = '/\[Multiple\]\[(.*)\](.*+)/';
                preg_match($re, $Input, $matches, PREG_OFFSET_CAPTURE);
                $Ex = explode(']', $Input);
                return [
                    'name' => $Ex[count($Ex) - 1],
                    'num' => $matches[1][0] ?? 1,
            ...
            if (in_array($Input, explode('|', $key))) {
                if (($value['type'] == -1 || $value['type'] == 2) && strstr($value['url'], '[url]')) {
...
';
    ...
    public static function ApplyList($Data)
    {
        ...
        $HoverList = [];
        $MenuList = [];
        $DataApp = array_slice($AppData['List'], $Page, $LIMIT); //取出本地安装的应用列表
        if (count($DataApp) >= 1) {
            $HoverList = self::HoverList(2);
            $MenuList = self::MenuList();
        }
        foreach ($DataApp as $value) {
            if ($HoverList && count($HoverList) !== 0 && in_array($value['identification'], $HoverList)) {
                $value['Hover'] = 1;
            } else {
                $value['Hover'] = 2;
            }
             ...
             * 检查是否开启了导航栏菜单
             */
            if (count($MenuList) !== 0 && isset($MenuList[$value['identification']])) {
                $value['MenuAdmin'] = $MenuList[$value['identification']]['MenuAdmin'] ?? 2;
                $value['MenuUser'] = $MenuList[$value['identification']]['MenuUser'] ?? 2;
                $value['MenuHome'] = $MenuList[$value['identification']]['MenuHome'] ?? 2;
            } else {
                $value['MenuAdmin'] = 2;
                $value['MenuUser'] = 2;
                $value['MenuHome'] = 2;
     ...
     * 取出专属访问链接！
     */
    public static function AppMenuURL($id, $type)
    {
        switch ($type) {
            case "User":
            case "Admin":
                return "ajax.php?act=app_view&id={$id}&path=index";
            default:
                return "main.php?act=AppView&id={$id}&path=index";
        }
    }

    //取出菜单列表
    public static function AppMenuList($type = 'MenuAdmin')
    {
        global $date;
        $Data = self::MenuList();
        $PayData = self::AppPaySync();
        $List = [];
        foreach ($Data as $key => $value) {
            if (!isset($value[$type]) || $value[$type] != 1) {
                continue;
            }
            $Conf = self::AppConf($key);
            if ($Conf['state'] == 2) {
                continue;
            }
            if (isset($PayData[$value['Data']['id']])) {
                if ($PayData[$value['Data']['id']]['endtime'] <= $date) {
                    continue;
                }
            }
            $List[] = [
     ...
     * 更新菜单列表数据！
     */
    public static function MenuSet($identification, $type, $state)
    {
        $File = ROOT . 'includes/lib/AppStore/Menu.json';
        $Data = [];
        if (is_file($File)) {
            $Data = json_decode(file_get_contents($File), TRUE);
        }
        $AppData = self::AppConf($identification);
        $Data[$identification][$type] = $state;
     ...
     * 未安装应用会过滤
     */
    public static function MenuList($name = false)
    {
        $flie_name = ROOT . 'includes/lib/AppStore/Menu.json'; //配置文件名
        if (!is_file($flie_name)) {
            return [];
        }
        ...
        $DataArr = [];
        foreach ($Data as $key => $value) {
            $dir = ROOT . 'includes/lib/soft/conf/' . $key;
    ...
    public static function install($identification, $Type = 1, $ret = false)
    {
        ...
        if ($flieState) {
            ...
            if ($FileType) {
                Hook::execute("PluginInstalSuccess", [
                    'identification' => $identification,
                    'type' => $Type, //1=安装，2=升级，3=手动上传安装包安装
                ]);
                if ($ret) {
                    return [
                        'code' => 1,
    ...
    {
        Hook::execute("StartUninstallPlugin", ['identification' => $identification]);
        $msg = '卸载状态：<br>';
        ...
        Hook::execute("EndUninstallPlugin", ['identification' => $identification]);
        dies(1, $msg);
    }
     ...
     * @param $path //访问目录！
     * @param $state //1=从主站后台访问，2=从用户后台或首页访问
     */
    public static function view($identification, $path, $state = 2)
    {
        ...
        if ($state == 2) {
            ...
            if (in_array('Admin', $ArrConf)) {
                if (!self::AdminVerify()) {
                    self::error(401, '无访问权限，此应用仅主站可访问！');
                }
            } else if (in_array('User', $ArrConf) && !self::UserVerify()) {
                self::error(401, '无访问权限，此应用仅登录用户可访问！');
            }
        }
        if ($vis['state'] != 1) {
     ...
     * @param array $Data
     * @param bool $Types
     * @param int $State //是否返回钩子内容
     * Api钩子,执行应用方法
     */
    public static function Api($identification = null, $Data = [], $Types = true, $State = 1)
    {
     ...
     * 应用验证
     */
    public static function AppVerify($identification)
    {
        $Data = Curl::Get('/api/AppStore/api', [
            'act' => 'Verification',
            'identification' => $identification,

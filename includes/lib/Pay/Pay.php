...
class Pay
{
    ...
    public static function PrepaidPhoneOrders($Data, $Goods = [])
    {
        ...
        $ID = $DB->id();
        if ($Res && $ID) {
            mkdirs(SYSTEM_ROOT . 'extend/log/Order/');
            mkdirs(SYSTEM_ROOT . 'extend/log/Pay/');
            if ($Data['gid'] != -1 && $Data['gid'] != -3) {
                //生成游客订单缓存
                query::OrderCookie($OrderNumber);
                //生成分站判定订单缓存文件
                UserConf::OrderDestinyDataWritten($OrderNumber);
            }
...
//        }
    ...
    {
        global $date;
        $Paths = SYSTEM_ROOT . 'extend/log/Order';
        mkdirs($Paths);
        if (is_file($Paths . '/PaySuccess_' . $Data['out_trade_no'] . '.log')) {
            return [
                'code' => 1,
                'msg' => '订单已回调成功,无需重复回调！',
            ];
        }
        file_put_contents($Paths . '/PaySuccess_' . $Data['out_trade_no'] . '.log', $Data['trade_no']);
        $lock = fopen($Paths . '/PaySuccess_' . $Data['out_trade_no'] . '.log', 'w');
        ...
        switch ((int)$Order['gid']) {
            ...
            case -1: //用户充值订单处理
                ...
                if ($Res) {
                    ...
                    $Rs = $DB->update('user', [
                        'money[+]' => round($Pids['price'], 2)
                    ], [
                        'id' => $Pids['uid'],
                    ]);
                    if ($Rs) {
                        userlog('在线充值', '用户' . $Pids['uid'] . '于' . $date . '成功充值' . round($Pids['price'], 2) . '元,支付ID:' . $Pids['id'] . '！', $Pids['uid'], round($Pids['price'], 2));
                        Hook::execute('PaySuccess', [
                            'PayOrder' => $Pids,
                            'type' => 3,
                        ]);
                        Hook::execute('BalanceRecharge', [
                            'id' => $Pids['id'],
                            'uid' => $Pids['uid'],
                            'money' => round($Pids['price'], 2),
                        ]);
                        ...
                        return [
                            'code' => 1,
                            'msg' => '恭喜您成功充值：' . round($Pids['price'], 2) . '元余额！'
                        ];
                    }
                    flock($lock, LOCK_UN);
                    fclose($lock);
                    return [
                        'code' => -1,
                        'msg' => '余额充值失败,请联系客服补款：' . round($Pids['price'], 2) . '元！'
                    ];
                }
                flock($lock, LOCK_UN);
                fclose($lock);

...
class App
{
    ...
    public static function AppDownload($id, $type = 1, $uid = -1)
    {
        ...
        if ((int)$type === 2) {
            ...
            if ($App['uid'] >= 1) {
                ...
                if (empty($UserData['configuration'])) {
                    $configuration_arr = [];
                } else $configuration_arr = json_decode($UserData['configuration'], true);

                $configuration_arr = array_merge($configuration_arr, [
                    'appurl' => ROOT_DIR . '?mod=AppDownload&id=' . $id,
                ]);
                $Res = $DB->update('user', [
                    'configuration' => json_encode($configuration_arr),
                ], [
                    'id' => $UserData['id']
                ]);
                if ($Res) {
        ...
        dier([
            'code' => 1,
            'msg' => $msg,
            'url' => href(2) . ROOT_DIR . '?mod=AppDownload&id=' . $id,
        ]);
    }
    ...
    public static function ImagePreview($ID)
    {
        ...
        if (file_exists($path . $ID . '.jpg')) {
            ...
            Header('Location: ' . href(2) . ROOT_DIR . '/includes/extend/log/App/image/' . $ID . '.jpg');
            die;
        }

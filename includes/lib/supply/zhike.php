...
class zhike
{
    ...
    public static function GoodsList($Data)
    {
        ...
        $header_response = @get_headers($SourceData['url'] . xiaochu_de('Lx3iNa0oYcXhRupyYuyn9mcm9udDMvY3NzL3NraW5fYmx1ZS5jc3MO0O0O'), 1);
        if ($header_response && strpos($header_response[0], "404") === false) {
            dies(-1, '近期收到不少用户举报，此<span style=color:red>货源系统(卡易信)</span>的部分站长人品很垃圾，为防止对接用户遭受跑路损失，已将此系统屏蔽，其中已成功对接的商品可以正常对接，但无法新增，直客系统可以正常对接，不受影响！');
        }
...
//            ];
        ...
        foreach ($Goods['paramsTemplate'] as $val) {
            if ($val['assist'] === 'multiply-number') {
                $Input[0][] = '[Multiple][1]' . $val['name']; //倍数乘积
            } else {
        ...
        return [
            ...
            'data' => [
                ...
                'money' => ((float)$Goods['goodsPrice'] * ($Goods['preUnitNum'] ?? $Goods['minOrderNum'])),
                'min' => 1,

...
class GoodsMonitoring
{
    ...
    public static function DataSynchronization($Goods, $Data, $type = 1, $state = 1)
    {
        ...
        if ($state === 1) {
            ...
            if ($Goods['state'] != 1) {
                //上架
                Hook::execute('GoodsShow', ['gid' => $Goods['gid'], 'name' => $Goods['name']]);
            }
        } else {
            //对接商品下架或无法购买状态！
            $GoodsSet = [
            ...
            if ($Goods['state'] != ($conf['SupervisoryError'] == 3 ? 2 : $Goods['state'])) {
                //下架
                Hook::execute('GoodsHide', ['gid' => $Goods['gid'], 'name' => $Goods['name']]);
            }
        }
    ...
    {
        Hook::execute('GoodsPriceFluctuate', ['gid' => $PriceChange['gid'], 'data' => $PriceChange]);
        global $accredit;

...
class skyun
{
    ...
    public static function Query($id, $TypeSupply)
    {
        ...
        $DataCurl = Api::Curl($TypeSupply['url'] . '/api/user/order/query', $DataPost);
        if (empty($DataCurl)) {
            return false;
        }
    ...
    public static function Submit($OrderData, $Goods, $TypeSupply)
    {
        ...
        $DataPost = array_merge([
            ...
            'api_pwd' => $TypeSupply['password'],
            'sid' => $GoodsData['gid'],
            'out_danhao' => $OrderData['order'],
            'num' => ($OrderData['num'] * $Goods['quantity']),
        ], $DataPost);
        $TypeSupply['url'] = StringCargo::UrlVerify($TypeSupply['url'], 3);

        $DataCurl = Api::SuppluCurl($TypeSupply['url'] . '/api/user/order', $DataPost);
    ...
    public static function CommodityStatus($Goods, $Supply)
    {
        ...
        $CurlData = Api::Curl($Supply['url'] . '/api/user/goods/info', $Post);
    ...
    public static function ProductDetails($Data)
    {
        ...
        $CurlData = Api::Curl($SourceData['url'] . '/api/user/goods/info', $Post);
...
//            ];
    ...
    public static function GoodsList($Data)
    {
        ...
        $CurlData = Api::Curl($SourceData['url'] . '/api/user/goods/list', $Post);

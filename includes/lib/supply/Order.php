...
class Order
{
        ...
        foreach (explode('|', $Goods['input']) as $key => $value) {
            if (strstr($value, '{') && strstr($value, '}')) {
                ...
                if ($InputArr[$key]['type'] == 3) {
                    ...
                    $Num = ($Sku[$key] - 0) ?? 1; //用户输入的倍数
                    $MultipleNum = $Num * $Multiple;
                    //赋予成本倍数
                    $Goods['money'] = $Goods['money'] * $MultipleNum;
                }
            }
        }
    ...
    public static function Creation($DataBuy, $User, $Queue = false, $Type = 1)
    {
        ...
        if ($Queue === false) {
            ...
            if ($Goods['freight'] != -1) {
                ...
                if ($Fre) {
                    $freight = Price::Freight($Fre, $DataBuy['data'], $Goods['price'], $DataBuy['num']);
                    $freight -= $Price;
                }
            }
            //倍数加价
        ...
        switch ((int)$DataBuy['type']) {
            ...
            case 2: //余额付款
                ...
                if ($Deduct) {
                    Hook::execute('PayMoney', [
                        'cause' => '余额支付,购买了商品' . $Goods['name'] . ',付款金额为：' . round($Price, 8) . '元',
                        'money' => $Price,
                        'uid' => $User['id'],
                    ]);
                    userlog('余额购买', '用户[' . $User['id'] . ']在' . $date . '用' . $Price . '元余额购买了商品[' . $Goods['name'] . ']！', $User['id'], $Price);
                    $OrData = self::OrderAwait($InitialPrice, $CouponId, '余额付款', '余额付款,无支付订单', $User['id'], $DataBuy, $Price, ($Money * $DataBuy['num']), false, 1);
                    if ($OrData == false) {
            ...
            case 3: //积分兑换
                ...
                if ($Deduct) {
                    Hook::execute('PayPoints', [
                        'cause' => '兑换了商品' . $Goods['name'] . ',消耗了' . $Price . $conf['currency'],
                        'money' => $Price,
                        'uid' => $User['id'],
                    ]);
                    userlog('积分兑换', '用户[' . $User['id'] . ']在' . $date . '用' . $Price . $conf['currency'] . '兑换了商品[' . $Goods['name'] . ']！', $User['id'], $Price);
                    $OrData = self::OrderAwait(-1, -1, '积分兑换', '积分兑换,无支付订单', $User['id'], $DataBuy, $Price, ($Money * $DataBuy['num']), false, 1);
                    if ($OrData == false) {
            ...
            case 4: //免费领取
                ...
                $OrData = self::OrderAwait(-1, -1, '免费领取', '免费商品,无支付订单', $User['id'], $DataBuy, 0, ($Money * $DataBuy['num']));
     ...
     * 下单参数验证模块
     */
    public static function VerifyBuy($Gid, $num, $InputData, $User = false, $DataRequest = [])
    {
        ...
        if ((float)$Goods['price'] <= 0 || ((count($DataRequest) >= 1 && (float)$Goods['points'] <= 0) || (int)($DataRequest['type'] ?? 1) === 3)) {
            /**
             * 此处验证免费商品和积分兑换商品
             */
     ...
     * 创建待处理订单，内置重复提交验证模块
     */
    public static function OrderAwait($InitialPrice, $Coupon, $payment, $trade_no, $Uid, $DataBuy, $Price, $Money, $OrderID = false, $Type = false, $TypeMsg = 1)
    {
        global $date, $conf;
        $DB = SQL::DB();
        if ($OrderID == false) {
        ...
        //获取分站信息
        $Uty = UserConf::judge(null, $trade_no);
    ...
    public static function PriceRefund($payment = false, $Type = false, $Uid = -1, $Price = 0, $DataBuy = [], $Msg = '', $Code = 1)
    {
        ...
        if ($Code == -1) {
            ...
            if ($Deducts) {
                userlog('订单退款', '用户[' . $Uid . ']在' . $date . '用' . $Price . ($payment == '余额付款' ? '元余额' : '点积分') . '购买了商品[' . $DataBuy['gid'] . ']，触发重复订单拦截模块，拦截金额' . $Price . ($payment == '余额付款' ? '元余额' : '点积分') . '已经自动补款到账户内！', $Uid, $Price);
                dies($Code, $Msg);
            } else {
                userlog('退款失败', '用户[' . $Uid . ']在' . $date . '用' . $Price . ($payment == '余额付款' ? '元余额' : '点积分') . '购买了商品[' . $DataBuy['gid'] . ']，触发重复订单拦截模块，拦截金额' . $Price . ($payment == '余额付款' ? '元余额' : '点积分') . '未补款到账户内, 请联系客服手动补款！', $Uid, $Price);
                dies($Code, $Msg);
            }
        }
    ...
    public static function OrderQueueId($ID)
    {
        ...
        if (!$Order) {
            ...
            if (!$Goods) return ['code' => -1, 'msg' => '此订单商品不存在,无法提交至服务器,请联系管理员处理！'];
            $User = $DB->get('user', '*', ['id' => (int)$Queue['uid']]);
            $Goods = self::VerifyBuy($Goods['gid'], $Queue['num'], json_decode($Queue['input'], TRUE), $User);

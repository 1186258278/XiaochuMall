...
class jiuqi
{
    ...
    {
        if ($Data['type'] != 2 && !empty($_SESSION['JiuQiGoodsList_' . $Data['id']])) {
            $GoodsList = [];
            foreach ($_SESSION['JiuQiGoodsList_' . $Data['id']] as $v) {
                $GoodsList[] = [
                    'gid' => $v['id'],
        ...
        if ($CurlDataJson['status'] === 1) {
            $_SESSION['JiuQiGoodsList_' . $Data['id']] = $CurlDataJson['data'];
            $GoodsList = [];
    ...
    public static function ProductDetails($Data)
    {
        ...
        if (empty($_SESSION['JiuQiGoodsList_' . $Data['sqid']])) {
            return [
                'code' => -1,
                'msg' => '数据源获取失败，请重新获取一遍可对接商品列表！',
            ];
        }
        $Goods = $_SESSION['JiuQiGoodsList_' . $Data['sqid']][$Data['key']];

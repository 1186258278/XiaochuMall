...
class xiaochu
{
    ...
    public static function verify($Data)
    {
        ...
        if (empty($Data['sign']) || empty($Data['url']) || empty($Data['id'])) {
    ...
    public static function GoodsDetails($User, $Gid)
    {
        ...
        $Goods = $DB->get('goods', [
            ...
            'date', 'selling', 'deliver', 'profit_rule'
        ], [
            'gid' => $Gid,
            'LIMIT' => 1,
            'method[~]' => '4',
        ...
        $GoodsPrice = Price::Get($Goods['money'], $Goods['profits'], $User['grade'], $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
        ...
        unset($Goods['method'], $Goods['profits'], $Goods['selling'], $Goods['deliver'], $Goods['profit_rule']);
    ...
    public static function Query($order, $TypeSupply)
    {
        ...
        $Data = $DataCurl['data'];
        if ($Data['token'] != -1 || is_array($Data['token'])) {
//卡密数据
            $DB = SQL::DB();
            $Input = json_decode($order['input'], true);
            $SQL = [];
            foreach ($Data['token'] as $v) {
                if ($v == '') {
                    continue;
                }
                $SQL[] = [

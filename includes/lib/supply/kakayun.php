...
//'dockapiorderno' => $Order['order']
        ...
        if ($DataCurl['code'] == 1) {
            ...
            return [
                'ApiType' => 1,
                'ApiNum' => $Data['buynum'],
                'ApiTime' => date('Y-m-d H:i:s', $Data['create_time']),
                'ApiInitial' => '无',
                'ApiPresent' => '无',
...
//'outorderno' => $OrderData['order'], //是否验证本地订单？防止重复提交
    ...
    public static function CommodityStatus($Goods, $Supply)
    {
        ...
        $CurlData = Api::Curl($Supply['url'] . 'dockapi/v2/goodsdetails.html', $Post);
        ...
        if ($GoodsDataApi['goodsstatus'] == 0) {
            return [
                'code' => 1,
                'msg' => '数据获取成功',
                'data' => [
                    'state' => 2,
                    'money' => $GoodsDataApi['goodsprice'],
    ...
    public static function ProductDetails($Data)
    {
        ...
        $CurlData = Api::Curl($SourceData['url'] . 'dockapi/v2/goodsdetails.html', $Post);
        $CurlDataJson = json_decode($CurlData, true);
        if (empty($CurlDataJson)) {
            return [
                'code' => -1,
...
//        }
        ...
        $Input = [];
        foreach ($Goods['attach'] as $key => $v) {
            $Input[0][] = $v['title'];
            $Input[1][] = 'input' . ($key + 1);
        }

        if (count($Goods['attach']) === 0) {
            $Input[0][] = '下单信息';
            $Input[1][] = 'input1';
        }
    ...
    public static function GoodsList($Data)
    {
        ...
        if ((int)$CurlDataJson['code'] === 1) {
//            $GoodsData = self::GoodsListHeavyLoad($Post, [
//                'password' => $SourceData['password'],
//                'url' => $SourceData['url'] . 'dockapi/v2/getallgoods.html',
//                'allpage' => $CurlDataJson['allpage'],
//            ]);
//            $GoodsList = array_merge($CurlDataJson['data'], $GoodsData);
            $GoodsList = [];
...
//数据重载
    public static function GoodsListHeavyLoad($Post, $Data, $page = 2)
    {
        sleep(2);
        $List = [];
        $Post['page'] = $page;
        $Post = array_merge($Post, [
            'sign' => self::getKkySign($Post, $Data['password']),
        ]);
        $CurlData = Api::Curl($Data['url'], $Post);
        $CurlDataJson = json_decode($CurlData, true);
        if (empty($CurlDataJson)) {
            return [];
        } else {
            $List = array_merge($List, $CurlDataJson['data']);
        }
        if ($page < $Data['allpage']) {
            $List = array_merge($List, self::GoodsListHeavyLoad($Post, $Data, $page + 1));
        }
        return $List;
    }
}

...
class jiuwu
{
    ...
    public static function GoodsList($Data)
    {
        ...
        if (empty($CurlDataJson)) {
            ...
            return [
                'code' => -1,
                'msg' => (!$Error ? '对接网站：' . $SourceData['url'] . ' 打开失败，请检查域名是否有勿，或手动Ping测试连通率' : '对接站点：' . $SourceData['url'] . ' 提示
<br>' . $Error),
            ];
        }
    ...
    public static function TakeOutTheData($data)
    {
        ...
        preg_match('/class="order_post_form".*?
<ul>(.*?)<\/ul>.*?<\/form>/us', $HtmlData, $tabs);
    ...
    public static function ProductDetails($Data)
    {
        ...
        return [
            'code' => 1,
            'msg' => '对接参数获取成功！' . ($His['balance'] ? '<br>当前账户余额：<font color=red>' . $His['balance'] . '元</font>' : '') . ($His['url'] ? ' <hr> <span
                        style="color:red">商品详情页：【' . $His['url'] . '】 打开失败，无法获取正确的对接信息和下单信息，请手动打开此页面获取！</span>' : ''),
            'data' => [
                'name' => $Goods['title'],
                'image' => $Goods['thumb'],
                'docs' => $His['docs'],

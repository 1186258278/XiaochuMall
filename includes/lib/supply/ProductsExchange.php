...
class ProductsExchange
{
    ...
    public static function Ordersubmit($DataBuy, $Token)
    {
        ...
        userlog('卡密兑换', ((!$User ? -1 : $User['id']) === -1 ? '游客' : '用户：' . (!$User ? -1 : $User['id'])) . '于' . $date . '使用了商品兑换卡兑换了商品,卡号为：' . $Token . '，对应的商品名称为：' . $Goods['gid'], (!$User ? -1 : $User['id']));
        $OrData = Order::OrderAwait(-1, -1, '卡密兑换[免费]', -1, (!$User ? -1 : $User['id']), $DataBuy, 0, ($Goods['money'] * $Goods['num']));

...
class Admin
{
     ...
     * 主站后台数据统计
     */
    public static function HomeData($type, $date)
    {
        global $times, $conf;
        self::SynchroServerDockInform();
    ...
    public static function UserRedact($_QET)
    {
        ...
        if ($Res) {
            ...
            //写入余额操作
            if ($_QET['field'] === 'money[+]' || $_QET['field'] === 'money[-]' || $_QET['field'] === 'currency[+]' || $_QET['field'] === 'currency[-]') {
                Hook::execute('BalanceOver', [
                    'uid' => $_QET['id'],
                    'type' => $_QET['field'],
                    'value' => $_QET['value'],
                ]);
    ...
    public static function GoodsPrice($Goods)
    {
        ...
        if ((int)$Goods['specification'] === 1) {
            $Name = 'F_' . md5((float)$Goods['profits'] . $Goods['money'] . $Goods['selling'] ?? '' . $Goods['profit_rule']);
            $PrEx = self::SpPrExists($Name);
            if ($PrEx) {
                return $PrEx;
            }
            unset($PrEx);
            $Pricer = Price::List($Goods['money'], $Goods['profits'], $Goods['gid'], $Goods['selling'], $Goods['profit_rule'], 2);
            if (count($Pricer) === 0) {
                $Min = $Goods['money'];
                $Max = $Goods['money'];
            } else {
    ...
    public static function SpPrice($Goods)
    {
        ...
        foreach ($SkuJson as $value) {
            ...
            if ($PrEx) {
                ...
                $Price = Price::Get($Money, $Goods['profits'], 1, $Goods['gid'], $Goods['selling'], $Goods['profit_rule'], 2);
    ...
    public static function GoodsAdd($Data)
    {
        ...
        $SQL = [
            ...
            'profit_rule' => $Data['profit_rule'] ?? -1,
            'label' => $Data['label'],
...
' . implode('=>', $Arr);
     ...
     * 检测是否添加了服务端对接通道
     */
    public static function SynchroServerDockInform()
    {
        global $date;
        $DB = SQL::DB();
        $Get = $DB->get('shequ', ['id'], [
    ...
    public static function TemConfSet($date)
    {
        ...
        if ($Res) {
            ...
            if ($date['json'] !== false) {
                //适配用户后台模板配置
                if ($date['field'] == 'UserBackgroundTemplatePC' || $date['field'] == 'UserBackgroundTemplateM') {
                    self::TemJsonSet(ROOT . "user/template/" . $date['value'] . "/conf.json", $_POST['json']);
                } else {
    ...
    public static function TemData()
    {
        ...
        $templateUser = for_dir(ROOT . "user/template/");
        $DataUser = [
            'PC' => [],
            'M' => [],
        ];
        foreach ($templateUser as $value) {
            if (!is_dir(ROOT . "user/template/" . $value)) {
                continue;
            }
            $file_path = ROOT . "user/template/" . $value . "/conf.json";
            if (!file_exists($file_path)) {
                $DataUser['PC'][$value] = false;
                $DataUser['M'][$value] = false;
            } else {
                $Json = json_decode(file_get_contents($file_path), true);
                if (!empty($Json)) {
                    if ((int)$Json['type'] == 1) {
                        $DataUser['PC'][$value] = $Json;
                    } else if ((int)$Json['type'] == 2) {
                        $DataUser['M'][$value] = $Json;
                    } else {
                        $DataUser['PC'][$value] = $Json;
                        $DataUser['M'][$value] = $Json;
                    }
        ...
        dier([
            ...
            'conf' => [
                ...
                'UserBackgroundTemplatePC' => $conf['UserBackgroundTemplatePC'], //PC端用户后台模板
                'UserBackgroundTemplateM' => $conf['UserBackgroundTemplateM'], //移动端用户后台模板
    ...
    /**
     * @param $cid //分类ID
     * @param $type //1=最顶部，2=上移一格，3=下移一格，4=最底部
     * 调整分类排序,二级分类适配版
     */
    public static function ClassSort($cid, $type)
    {
        $DB = SQL::DB();
        $Class = $DB->get('class', ['cid', 'sort', 'superior'], [
            'cid' => $cid,
        ]);
        if (!$Class) {
            dies(-1, '分类不存在！');
        }
        $Count = $DB->count('class', [
            'superior[' . ($Class['superior'] >= 1 ? '>' : '<') . ']' => 0
        ]);
        //DESC 降序，ASC 升序
        if ($type == 2 || $type == 3) {
            $Select = $DB->select('class', ['cid'], [
                'superior[' . ($Class['superior'] >= 1 ? '>' : '<') . ']' => 0,
                'ORDER' => [
                    'sort' => 'DESC'
                ],
        ...
        switch ($type) {
            case 1: //最顶部
                if ($Count > 1) {
                    ...
                    foreach (array_reverse($Select) as $key => $value) {
                        $DB->update('class', [
                            'sort' => $key + 1,
                        ], [
                            'cid' => $value['cid'],
                ...
                return $DB->update('class', ['sort' => $Count], ['cid' => $cid]);
            case 2: //上移一格
                if ($Count > 1) {
                    if (count($List) >= 2 && $index != 0) {
                        $cid1 = $List[$index];
                        $cid2 = $List[$index - 1];
                        $List[$index - 1] = $cid1;
                        $List[$index] = $cid2;
            ...
            case 3: //下移一格
                if ($Count > 1) {
                    if (count($List) >= 2 && ($index + 1) != count($List)) {
                        $cid1 = $List[$index];
                        $cid2 = $List[$index + 1];
                        $List[$index + 1] = $cid1;
                        $List[$index] = $cid2;
            ...
            default: //最底部
                ...
                return $DB->update('class', ['sort' => 1], ['cid' => $cid]);
        }
    }
}

...
function dies($code = -1, $msg = '', $type = 1)
{
    ...
    ob_end_clean();
    ob_start();
    if (empty($array['HookName'])) {
        $array = SonCallbackSet($array, 1);
    } else {
...
function dier($arr = [], $type = 1)
{
    ...
    ob_end_clean();
    ob_start();
    if (empty($arr['HookName'])) {
        $arr = SonCallbackSet($arr, 2);
    } else {
        unset($arr['HookName']);
    }
...
/**
 * @return never
 * 调用钩子队列，并且让请求先返回，后执行，实现无堵塞执行钩子
 */
function CallHookQueue()
{
    @header("Connection: close");
    @header('Content-Length: ' . ob_get_length());
    ob_end_flush();
    require_once __DIR__ . '/extend/HookQueue.php';
    @HookQueue::PerformTask();
    die;
}
 ...
 * @param $str //需要筛选的字符串
 * @param $type //1=排除，2=包含
 * @return array
 * 获取文件夹下的文件夹列表，1层
 * 获取目录列表
 * 获取文件列表
 */
function for_dir($dir, $exclude = [], $str = false, $type = 1)
{
    $files = array();
...
function show_msg($title = '温馨提示', $content = '这是内容', $type = 3, $url = false, $true = true, $w = 42, $h = 42)
{
    ...
    ob_end_clean();
    ob_start();
    echo str_replace('[Content]', $content, $Data);
    CallHookQueue();
}
...
class config
{
    ...
    {
        $config = Maintain::Caching();
        $Arr = array_merge($config, self::Notice());
    ...
    {
        return Maintain::clearGlobalEach();
    }
}
...
class reward
{
    ...
    public static function user_landing()
    {
        ...
        if (!empty($_COOKIE['THEKEY'])) {
            $idu = SecurityFiltering($_COOKIE['THEKEY'] ?? 0);
            if (strlen($idu) === 32) {
...
class login_data
{
    ...
    public static function user_grade($uid)
    {
        ...
        if ($conf['PermissionTime'] == 1 && !empty($G['endtime']) && $G['endtime'] < $date) {
            $G['grade'] = 1;
        }
    ...
    public static function user_data()
    {
        ...
        if (!empty($idu)) {
            ...
            //权限已经到期
            if ($conf['PermissionTime'] == 1 && !empty($user_row['endtime']) && $user_row['endtime'] < $date) {
                $user_row['grade'] = 1;
            }
 ...
 * 数据验证,文件缓存模块
 */
function HomecachingVerify($act, $cs = [], $time = false)
{
    $cs["hosturls"] = href();
    if (!$time) {
        global $conf;
        $time = $conf['Homecaching'] - 0;
    }
    $Flie = ROOT . 'includes/extend/log/Home/Cache_' . md5($act . json_encode($cs)) . '.cache';
    if (file_exists($Flie)) {
 ...
 * 模板配置读取
 */
function TemConf($name, $type = 1, $state = 1)
{
    if ($state == 2) {
        $file_path = ROOT . 'user/template/' . $name . '/conf.json';
    } else {
...
//取出倍数名称
function TakeMultipleNames($value)
{
    if (isset(CookieCache::$Cache['TakeMultipleNames'][$value])) {
        return CookieCache::$Cache['TakeMultipleNames'][$value];
    }
    $data = AppList::MatchingInput($value);
    if (!$data) {
        CookieCache::$Cache['TakeMultipleNames'][$value] = $value;
        return $value;
    }
    //倍数
    if ($data['type'] === 3) {
        CookieCache::$Cache['TakeMultipleNames'][$value] = $data['name'];
        return $data['name'];
    }
    CookieCache::$Cache['TakeMultipleNames'][$value] = $value;
    return $value;
 ...
 * 订单改价钩子调用方法集
 */
function ChangOrderPrice($Data)
{
    $Tis = Hook::execute('ChangOrderPrice', $Data, 2);
    if (!$Tis || $Tis['code'] != 2 || count($Tis['data']) === 0) {
        return [
            'Price' => $Data['Price'],
            'Points' => $Data['Points'],
        ];
    }
    //最终的价格,按照插件安装顺序来判断,以最晚安装的插件为准！
    $ApiData = $Tis['data'][count($Tis['data']) - 1]['Data'];
    if (empty($ApiData)) {
        return [
            'Price' => $Data['Price'],
            'Points' => $Data['Points'],
 ...
 * 订单详情扩展集
 */
function OrderDetailExtent($Data)
{
    $Tis = Hook::execute('OrderDetailExtent', $Data, 2);
    if (!$Tis || $Tis['code'] != 2 || count($Tis['data']) === 0) {
        return [];
    }
    $List = $Tis['data'];
    ...
    foreach ($List as $Value) {
        if (empty($Value['Data']) || !is_array($Value['Data']) || count($Value['Data']) === 0) {
            continue;
        }
 ...
 * 数据回调扩展集
 */
function SonCallbackSet($Data, $type)
{
    global $_QET;
    $Hook = Hook::arr('SonCallback');
    $Count = count($Hook);
    if ($Count === 0) {
        return $Data;
    }
    //此处加入判断参数，防止重复调用
    $DataIncoming = array_merge($Data, [
        'HookName' => 'SonCallback'
    ]);
    //开始执行钩子
    foreach ($Hook as $value) {
        $Api = AppList::Api($value, [
            'data' => $DataIncoming,
            'type' => $type,
            'QET' => $_QET,
            'HookName' => 'SonCallback',
        ], false, 2);
        if ($Api && !empty($Api['data']) && is_array($Api['data'])) {
            $DataIncoming = $Api['data'];
            $DataIncoming['HookName'] = 'SonCallback';
        }
        unset($Api);
    }
    unset($DataIncoming['HookName']);
    return $DataIncoming;
}

/**
 * @return bool
 * 升级数据校准模块,每次升级完成后自动校准数据库！
 */
function UpgradeDataCalibreModel()
{
    $FileGet = ROOT . 'install/install.lock';
    if (!is_file($FileGet)) {
        return false;
    }
    global $accredit;
    $Text = file_get_contents($FileGet);
    if ($Text === $accredit['versions']) {
        return true;
    }
    Maintain::run();
    if (file_put_contents($FileGet, $accredit['versions'])) {
        return true;
    }
    return false;
}
...
//加价规则缓存
$PriceIncreaseAccordCommonRule = [];
/**
 * @param $Money //成本
 * @param $SingleRule //规则编号
 * @param $Level //用户等级
 * @param $Type //1=价格，2=积分
 * 商品规则加价
 */
function PriceIncreaseAccordCommonRule($Money, $SingleRule, $Level, $Type = 1)
{
    $NameToken = md5($Money . $SingleRule . $Level . $Type);
    if (!empty($PriceIncreaseAccordCommonRule[$NameToken])) {
        return $PriceIncreaseAccordCommonRule[$NameToken];
    }
    $DB = SQL::DB();
    $Res = $DB->get('single_rule', [
        'type', 'json',
    ], [
        'state' => 1,
        'id' => $SingleRule,
    ...
    if (!$Res) {
        $PriceIncreaseAccordCommonRule[$NameToken] = false;
        return false;
    }
    ...
    //1=固定加价，2=百分比加价
    if ($Res['type'] == 1) {
        $Moneys = $Money + $Rules[($Type == 1 ? 'price' : 'profits')];
        $PriceIncreaseAccordCommonRule[$NameToken] = $Moneys;
        return $Moneys;
    }
    $Moneys = $Money + ($Money * ($Rules[($Type == 1 ? 'price' : 'profits')] / 100));
    $PriceIncreaseAccordCommonRule[$NameToken] = $Moneys;
    return $Moneys;
}

...
class HomePage
{
    ...
    public static function GoodsList($DataRe)
    {
        ...
        $DataRe['Sorted'] = $DataRe['Sorted'] ?? 1;

        if (!self::$User || empty(self::$User['grade'])) {
            $grade = 1;
        } else {
            $grade = (int)self::$User['grade'] ?? 1;
        }
        ...
        $SQL = [
            'goods.state' => 1,
            'ORDER' => [
                'goods.' . ($conf['SortingRules'] ?? 'sort') => ($DataRe['Sorted'] == 2 ? 'ASC' : 'DESC')
            ]
        ];

        if (isset($DataRe['page']) && (int)$DataRe['page'] >= 1) {
            if (empty($DataRe['limit'])) {
        ...
        if (isset($DataRe['cid']) && (int)$DataRe['cid'] <= 0) {
            unset($DataRe['cid']);
        }
        ...
        if (isset($DataRe['SortingType'])) {
            switch ((int)$DataRe['SortingType']) {
                case 2: //价格排序 money
                    $SQL['ORDER'] = [
                        'goods.money' => ($DataRe['Sorted'] == 2 ? 'ASC' : 'DESC')
                    ];
                    break;
                case 3: //根据库存 quota
                    $SQL['ORDER'] = [
                        'goods.quota' => ($DataRe['Sorted'] == 2 ? 'ASC' : 'DESC')
                    ];
                    break;
                case 4: //类型 cid
                    $SQL['ORDER'] = [
                        'goods.cid' => ($DataRe['Sorted'] == 2 ? 'ASC' : 'DESC')
                    ];
                    break;
            }
        }
        ...
        $Res = $DB->select('goods', [
            ...
            'quantity', 'quota', 'deliver', 'label', 'units', 'sales', 'selling', 'accuracy', 'profit_rule'
        ], $SQL);
        ...
        $Data = [];
        foreach ($Res as $res) {
            $Price = Price::Get($res['money'], $res['profits'], (self::$User === false ? -1 : self::$User['grade']), $res['gid'], $res['selling'], $res['profit_rule']);
            $res['quota'] -= 0;
            ...
            $res['method'] = PaymentMethodAnalysis(json_decode($res['method']));
    ...
    public static function ProductDetails($Data)
    {
        ...
        if ((int)$conf['LevelDisplay'] === 1) {
            $List = Price::List($Res['money'], $Res['profits'], $Res['gid'], $Res['selling'], $Res['profit_rule']);
            $PriceList = [];
            $Grade = (self::$User === false ? 1 : self::$User['grade']);
            if ($Grade > count($List)) {
                $Grade = count($List);
            }
            foreach ($List as $k => $v) {
...
//            ];
    ...
    public static function CreateOrder($Data)
    {
        ...
        if ((int)$Goods['freight'] !== -1) {
            ...
            if ($Fre) {
                $freight = Price::Freight($Fre, $Data, $Goods['price'], $Goods['num']);
                $freight -= $price;
            }
        }
        ...
        foreach ($InputArr as $key => $value) {
            ...
            //下拉选择框参数验证
            if ($value['type'] == 888) {
                if (!in_array($Data[$key + $I], $value['data'])) {
        ...
        if ($Multiple) {
            $points *= ($MultipleData['num'] * $Data[$MultipleData['index']]);
            $price *= ($MultipleData['num'] * $Data[$MultipleData['index']]);
        }
        //商品价格加运费
        $price += $freight;
        ...
        $ChangOp = [
            ...
            'Data' => $Data,
            'Gid' => $Gid,
            'Uid' => (!self::$User ? -1 : (self::$User['id'] - 0)),
            'Num' => $Num,
            'Mode' => 6,
            'Type' => 1,
        ];
    ...
    public static function ChangesCommodityPricesGoodsList($Gid)
    {
        ...
        $DB = SQL::DB();
        $Goods = $DB->get('goods', [
            'profits', 'selling', 'gid', 'image', 'state', 'profit_rule'
        ], [
            'gid' => (int)$Gid,
        ]);
        if (!$Goods) {
        ...
        foreach ($PriceChangeData as $val) {
            $NewPrice = Price::Get($val['money'], $Goods['profits'], $Leve, $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
            $UsedPrice = Price::Get($val['OriginalPrice'], $Goods['profits'], $Leve, $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
    ...
    public static function ChangesCommodityPricesList($Name)
    {
        ...
        if (empty($Name) || $Name == -1 || !is_file(ROOT . 'includes/extend/log/PriceChange/' . $KeyName . $Name . '.log')) {
        ...
        $List = [];
        $i = 0;
        foreach (array_reverse($ListArr) as $v) {
            if ($i >= 12 || empty($v) || $v === '.' || $v === '..') {
                continue;
            }
            ++$i;
    ...
    {
        if ($User['state'] != 1) {
            dies(-1, '您当前账户状态异常，请联系客服处理！');
        }
        global $conf, $times, $date;
        $DB = SQL::DB();

        $Ug = $DB->get('user', ['grade'], ['id' => $User['id']]);
        $Pru = $User;
        $Pru['grade'] = $Ug['grade'];
        $Giv = RatingParameters($Pru);
        ...
        if ($conf['userleague'] == 1 && $User['grade'] >= $conf['userleaguegrade'] && $conf['userappstate'] == 1) {
        ...
        //自定义导航
        $List_1_1 = [];
        $DataMenu = AppList::AppMenuList("MenuUser");
        foreach ($DataMenu as $val) {
            $List_1_1[] = [
                'name' => $val['name'], 'icon' => $val['image'], 'path' => href(2) . ROOT_DIR_S . '/user/ajax.php?act=app_view&id=' . $val['id'] . '&path=index', 'type' => 3,
                'data' => ['type' => 'warning', 'text' => '', 'state' => false]
            ];
        }
        ...
        $List_2[] = [
            ...
            'data' => ['type' => 'success', 'text' => '可领取:' . $inviteCount . $conf['currency'], 'state' => $inviteCount > 0]
        ];
        ...
        $List_2[] = [
            'name' => '退出登录', 'icon' => href(2) . ROOT_DIR . 'assets/img/user/b7.png', 'path' => 'Quit', 'type' => 4,
            'data' => ['type' => 'error', 'text' => '', 'state' => false]
        ];

        $Lists = [$List_1, $List_2];
        if (count($List_1_1) >= 1) {
            $Lists = [$List_1, $List_1_1, $List_2];
        }
        if (!empty($User['endtime'])) {
            $Price = ($GivS['money'] ?? 100) * ($conf['PermissionRenewPrice'] / 100);
            $Price = round($Price, 2);
        } else {
            $Price = 100;
        }
        ...
        //提现开关
        if ($conf['userdeposit'] == 1 && $User['grade'] >= $conf['userdepositgrade']) {
            $withdrawSwitch = true;
        } else {
        ...
        if ($conf['userdomaintype'] == 1) {
            ...
            if (!empty($User['domain'])) {
                $Domain = is_https(false) . $User['domain'];
            }
        } else {
            $Domain = $User['domain'];
            if (empty($Domain)) {
        ...
        $expireState = true; //默认未到期
        if (!empty($User['endtime']) && $User['endtime'] < $date) {
            $expireState = false; //已经到期
        }
        ...
        dier([
            ...
            'conf' => [ //控制开关
                'currency' => $conf['currency'],
                'userleaguegrade' => $conf['userleaguegrade'], //分站门槛
                'CartState' => $conf['CartState'] == 1, //购物车开关
                'FluctuationsPrices' => $conf['FluctuationsPrices'] == 1, //价格波动配置
            ],
            'order' => [ //订单配置
                'a1' => 0,//$DB->count('order', ['uid' => $User['id']]), //全部
            ...
            'data' => [
                ...
                'RecordNumber' => $conf['RecordNumber'] ?? false,
                'UserBackgroundTips' => (empty($conf['UserBackgroundTips']) ? false : explode('-', $conf['UserBackgroundTips'])), //用户后台小提示,留空隐藏
                'endtime' => $User['endtime'] ?? false, //权限到期时间
                'PermissionTime' => $conf['PermissionTime'], //是否开启了权限到期时间
                'PermissionRenewPrice' => $Price, //续期多少元 $GivS
                'PermissionLengthTime' => $conf['PermissionLengthTime'], //续期多少天
                'withdrawSwitch' => $withdrawSwitch, //提现开关
                'subStatSwitch' => ($Giv['min'] >= $conf['userleaguegrade']), //分站开关
                'expireState' => $expireState, //是否到期
            ]
        ]);
    }
}

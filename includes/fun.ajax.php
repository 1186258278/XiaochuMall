...
function LabelaAnalysis($Data)
{
    ...
    if (!empty(CookieCache::$Cache['LabelaAnalysis'][md5($LabelStr)])) {
        return CookieCache::$Cache['LabelaAnalysis'][md5($LabelStr)];
    }
    global $conf;
    if (empty($Data)) {
    ...
    CookieCache::$Cache['LabelaAnalysis'][md5($LabelStr)] = $Label;
    return $Label;
}
...
{
    $CountAr = HomecachingVerify('CommoditySalesAnalysis_', [$Gid, $sales], 60);
    if ($CountAr) {
        return $CountAr['count'] - 0;
    }
    global $conf;
    ...
    if (empty($_SESSION['GIDSALES_' . $Gid])) {
        $SALES = $sums + round(($sums * (rand(1, 50) / 100)));
        $_SESSION['GIDSALES_' . $Gid] = $SALES;
    } else {
        $SALES = $_SESSION['GIDSALES_' . $Gid];
    }
    $Count = $DB->sum('order', 'num', ['gid' => $Gid]) + $SALES;
    HomecachingAdd('CommoditySalesAnalysis_', [$Gid, $sales], ['count' => $Count]);
    return $Count;
}
...
function RlueAnalysis($Goods = [], $type = 2, $User = false, $pushMoney = false)
{
    ...
    foreach ($SkuJson as $key => $value) {
        ...
        if ($pushMoney === false) {
            $PriceList = Price::List($value['money'], $Goods['profits'], $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
            $Price = Price::Get($value['money'], $Goods['profits'], (!$User ? 1 : $User['grade']), $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
        } else {
            $PriceList = Price::List($value['money'], $Goods['profits'], $Goods['gid'], $Goods['selling'], $Goods['profit_rule'], 2);
            $Price = Price::Get($value['money'], $Goods['profits'], (!$User ? 1 : $User['grade']), $Goods['gid'], $Goods['selling'], $Goods['profit_rule'], 2);
        }
...
class query
{
    ...
    public static function QueryList($val)
    {
        ...
        if (strstr($val, '#')) {
            $Ex = explode('#', $val);
            $SQL['OR']['order.input[~]'] = $Ex[0] ?? $Ex[1] ?? $Ex[2] ?? $Ex[3];
        }
        ...
        foreach ($Res as $value) {
            ...
            if ($Input[0] != $val && $value['order'] != $val) continue;
            unset($Input);
            $Input = [];
            ...
            foreach (explode('|', $value['value']) as $k => $v) {
                if (strstr($v, '{') && strstr($v, '}')) {
                    ...
                    $Input[] = TakeMultipleNames(empty($v) ? '输入框' . ($k + 1) : $v);
                }
            }
    ...
    public static function QueryTake($ID, $User)
    {
        ...
        if ($Order['state'] != 1 && $Order['take'] != 1) {
            dies(-1, '此订单无法调整为收货状态，已完成的订单才能确认收货哦');
        }
        $Res = $DB->update('order', [
            'take' => 2,
        ], [
        ...
        if ($Res) {
            ...
            $Order['take'] = 2;
            //写入提成信息
            $Path = SYSTEM_ROOT . 'extend/log/PushMoney/' . $Order['order'] . '.data';
            if (is_file($Path)) {
                $Data = json_decode(file_get_contents($Path), true);
                UserConf::PushMoney($Data['Muid'], $Data['TradeNo'], $Data['Uid'], $Data['order'], $Data['gid'], (float)$Data['price'], $Data['type'], $Data['num'], json_decode($Data['input'], TRUE), 2);
                unlink($Path);
            }
    ...
    public static function VisitorsOrder()
    {
        ...
        foreach ($Res as $v) {
            ...
            //移除中文,防止恶意查单
            if (preg_match_all("/^([\x81-\xfe][\x40-\xfe])+$/", $v['trade_no'])) {
                continue;
            }
            $Data[] = $v['trade_no'];
        }
    ...
    public static function OrderUser($uid)
    {
        ...
        if (isset($_COOKIE['QUERYORDER'])) {
            ...
            setcookie("QUERYORDER", "", time() - 60 * 60 * 24 * 30, '/');
            setcookie("QUEUEORDER", "", time() - 60 * 60 * 24 * 30, '/');
            return true;
        } else return false;
    }
    ...
    public static function OrderPay($id)
    {
        ...
        switch ($re['payment']) {
            ...
            case '积分兑换':
                ...
                break;
            case '免费领取':
                userlog('免费领取', '您于' . $date . '领取了商品' . $Goods['name'], $UserData['id'], 0);
                $Re = $DB->update('queue', [
                    'type' => 2,
                    'remark' => "当前已经免费领取了商品,系统会自动提交订单,请耐心等待！",
                    'trade_no' => '免费领取无支付订单'
                ...
                if ($Re) {
                    Hook::execute('PayFree', [
                        'cause' => '订单队列补单，免费领取了商品' . $Goods['name'],
                        'uid' => $UserData['id'],
                    ]);
                    dies(1, '领取成功,点击刷新！');
                } else dies(-1, '订单状态调整失败，若扣款请联系管理员处理！');
    ...
    public static function OrderAll($id = false, $msg = '', $state = 1, $Page = 1, $User = false, $limit = 6)
    {
        ...
        if (!$User) {
            $Array = self::VisitorsOrder();
            $SQL = [
                'order.trade_no' => (count($Array) >= 1 ? $Array : -1000000000),
                'LIMIT' => [$Page, $LIMIT],
                'ORDER' => [
                    'order.id' => 'DESC'
                ],

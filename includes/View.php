...
class View
{
    ...
    public static function Home($mod = 0)
    {
        ...
        if (!empty($mod)) {
            //载入扩展模块
            require_once SYSTEM_ROOT . "Curl.php";
        }
    ...
    public static function AntiReptile()
    {
        ...
        if (!empty($_QET[$Key])) {
            setcookie("AntiReptile", xiaochu_en($Key, $accredit['token']), time() + 86400);
            die('<script type="text/javascript">window.location.href="' . ROOT_DIR . '?' . xiaochu_de($_QET[$Key]) . '";</script>');
        }
        if (!isset($_COOKIE['AntiReptile']) || $_COOKIE['AntiReptile'] != xiaochu_en($Key, $accredit['token'])) {
            $url = ROOT_DIR . (empty($_SERVER['QUERY_STRING']) ? '?' . $Key . '=' : '?' . $_SERVER['QUERY_STRING'] . '&' . $Key . '=') . xiaochu_en($_SERVER['QUERY_STRING']);
    ...
    {
        if (self::isPost() || isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
            return true;
        } else {
            return false;
        }

...
class Server
{
     ...
     * 主机在线付款续期！
     */
    public static function HostPay($price, $type, $name, $day, $MainframeData)
    {
    ...
    public static function Renewal($id, $Day = 30)
    {
        ...
        if ($re['status'] === true) {
            ...
            Hook::execute('HostRenew', [
                'id' => $MainframeData['id'],
                'previousExpireTime' => $MainframeData['endtime'],
                'expireTime' => $DataEnd,
            ]);
    ...
    {
        Hook::execute('HostExitLogin', [
            'identification' => $_SESSION[self::$SessionName],
        ]);
    ...
    public static function LoginVerification($Data)
    {
        ...
        global $conf;
        if (empty($_SESSION['LoginHost']) || $_SESSION['LoginHost'] !== md5(trim($Data['vercode']) . href()) && $conf['VerificationCodeSwitch'] != 2) {

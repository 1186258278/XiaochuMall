...
class OnebuttonDockingSystem
{
    ...
    public static function GoodsList($Data, $type = 1, $level = 1)
    {
        ...
        foreach ($GoodsList as $Goods) {
            ...
            unset($Goods['selling'], $DataImage, $Goods['explain'], $Goods['sales'], $Goods['sqid'], $Goods['note'], $Goods['profits'], $Goods['deliver'], $Parameter, $Goods['profit_rule']);
            $List[] = $Goods;
            unset($Goods);
        }
    ...
    public static function Test($Data)
    {
        ...
        $DataPost = [
            'url' => href(),
            'id' => $Data['uid'],
            'act' => 'DockingClassList'
        ];
        $DataPost = array_merge([
            'sign' => self::Sign($DataPost, $Data['token'])
        ], $DataPost);
        $ClassList = Api::Curl($Data['domain'] . 'api.php', $DataPost);
        if (!$ClassList) {
            $ClassList = [];
            $ClassList['data'] = [];
//            return [
//                'code' => -1,
//                'msg' => '分类列表获取失败，请确保对方站点版本和您的版本一致，并且存在分类列表！'
//            ];
        }

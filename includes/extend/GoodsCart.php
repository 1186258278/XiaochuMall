...
class GoodsCart
{
     ...
     * 创建队列订单，正常下单模式
     */
    public static function OredeQueue($Datas, $GidData, $uid, $PriceData)
    {
        global $conf, $times;
        $DB = SQL::DB();
        if ($Datas['type'] == 1) {
        ...
        if ($price === 0) {
            ...
            $Datas['CouponId'] = $PriceData['CouponId'];
        }
        ...
        if (!empty($Datas['CouponId']) && $Datas['CouponId'] != -1 && $GidData['gid'] >= 1 && $uid >= 1) {
            ...
            if ($conf['CouponMinimumType'] == 1) {
                if ($PriceCou <= ($PriceData['money'] * $Datas['num'])) {
                    $PriceCou = ($PriceData['money'] * $Datas['num']);
                }
            } else {
                if ($PriceCou <= 0) {
                    $PriceCou = 0;
    ...
    public static function CartPay($DataArr = [], $type = 1, $mode = 'alipay', $CartKey = -1)
    {
        ...
        foreach ($Array['data'] as $v) {
            if ((int)$v['GidData']['quota'] < (int)$v['GidData']['Count']) {
                $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '当前库存不足' . $v['GidData']['Count'] . '份,当前仅剩' . $v['GidData']['quota'] . '份,无法完成结算，';
                unset($CartDet[$i - 1]);
                ++$i;
                continue;
            }
            ...
            if ((int)$v['num'] <= 0) {
                $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '下单数量异常,无法完成结算，';
                unset($CartDet[$i - 1]);
                ++$i;
                continue;
            }
            ...
            if ($v['GidData']['communityId'] == '3') {
                ...
                if ($count_kami < (int)$v['GidData']['Count']) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '卡密库存总数不足' . $v['GidData']['Count'] . '个,无法完成结算,当前仅剩' . $count_kami . '张卡，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
            ...
            if ($payment === '积分付款') {
                if ($UserData == false) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '此商品必须登陆才可兑换哦,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
                ...
                if (($B + $v['count']) > $conf['getinreturn']) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '，购物车内此商品总数为：' . $v['count'] . '份！您今日最多可兑换：' . $conf['getinreturn'] . '份！,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
                ...
                if (($B + $v['count']) > $conf['getinreturn_all']) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '本站今日可兑换次数已经耗尽,本站今日提供的' . $conf['getinreturn_all'] . '此商品兑换次数不能满足当前商品下单总数(' . $v['GidData']['Count'] . ')！,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
            ...
            if ($v['GidData']['price'] === 0 && $v['GidData']['points'] === 0) {
                if (!$UserData) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '此商品必须登陆才可领取哦,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
                ...
                if (($Day + $v['Charge']) > (int)$conf['getinreturn']) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '，购物车内此商品总数为：' . $v['Charge'] . '份！您今日最多可领取：' . $conf['getinreturn'] . '份！,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
                ...
                if (($Day + $v['Charge']) > (int)$conf['getinreturn_all']) {
                    $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '本站今日可领取次数已经耗尽,本站今日提供的' . $conf['getinreturn_all'] . '次商品领取次数不能满足当前商品下单总数！,无法完成结算，';
                    unset($CartDet[$i - 1]);
                    ++$i;
                    continue;
                }
            ...
            if ($Data['payment'] !== '免费领取') {
                ...
                if ($payment === '积分兑换') {
                    if ((!in_array(3, json_decode($v['GidData']['method'], TRUE)))) {
                        $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '，不支持积分兑换，无法完成结算！，';
                        unset($CartDet[$i - 1]);
                        ++$i;
                        continue;
                    }
                    ...
                    if ((!in_array(2, json_decode($v['GidData']['method'], TRUE)))) {
                        $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '，不支持余额购买，无法完成结算！，';
                        unset($CartDet[$i - 1]);
                        ++$i;
                        continue;
                    }
                    ...
                    if ((!in_array(1, json_decode($v['GidData']['method'], TRUE)))) {
                        $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '，不支持在线付款，无法完成结算！，';
                        unset($CartDet[$i - 1]);
                        ++$i;
                        continue;
                    }
            ...
            if ($re == false) {
                $text .= '第' . $i . '件商品：' . $v['GidData']['name'] . '订单队列创建失败,无法完成结算，';
                unset($CartDet[$i - 1]);
                ++$i;
                continue;
            }
            ...
            case 2: //余额付款
                ...
                if ($Res) {
                    ...
                    if ($re) {
                        ...
                        dies(1, '购买成功,点击查看您的商品订单！' . (empty($text) ? '' : '，' . $text));
    ...
    public static function CartList($array = [], $CartKey = -1, $User = -2)
    {
        ...
        foreach ($DataArr as $key => $v) {
            ...
            $Seckill = 'Seckill' . $v['gid']; //秒杀商品
            $Multiple = 'Multiple' . $v['gid']; //基础加价倍数,需要于用户倍数相乘
            ...
            if (empty($$Gid)) {
                ...
                //倍数加价
                $$Multiple = false;
                foreach (explode('|', $$Gid['input']) as $value) {
                    if (strstr($value, '{') && strstr($value, '}')) {
                        continue;
                    } else {
                        $Matc = AppList::MatchingInput($value);
                        if ($Matc && $Matc['type'] == 3) {
                            $$Multiple = (int)$Matc['num']; //需要加价的倍数
                        }
                        unset($Matc);
                    }
                }
            ...
            $Multiple = $$Multiple; //商品基础加价倍数，如果为false，则无需加价

            /**
             * 开始计算商品价格
             */
            if ((int)$Goods['specification'] === 1) {
                $Price = Price::Get($Goods['money'], $Goods['profits'], login_data::user_grade(($User === false ? -1 : $User['id'])), $Goods['gid'], $Goods['selling'], $Goods['profit_rule']);
            } else {
            ...
            if ((int)$Goods['freight'] !== -1) {
                $freight = Price::Freight($$Fhuo, $v['input'], $Price['price'], $v['num']);
                $freight -= $Price['price'];
            }
            //计算商品价格
            ...
            //开始计算倍数加价
            if ($Multiple) {
                foreach (explode('|', $Goods['input']) as $kes => $value) {
                    if (strstr($value, '{') && strstr($value, '}')) {
                        continue;
                    } else {
                        $Matc = AppList::MatchingInput($value);
                        if ($Matc && $Matc['type'] == 3) {
                            $Price['price'] *= ($Multiple * $v['input'][$kes]);
                            $Price['points'] *= ($Multiple * $v['input'][$kes]);
                        }
                        unset($Matc);
                    }
                }
            ...
            $v['GidData'] = [
                ...
                'points' => round(($Price['points'] < 0 ? 1 : $Price['points'])),
                'cid' => (int)$Goods['cid'],
    ...
    public static function CartAdd($Data)
    {
        ...
        $Goods = Order::VerifyBuy($Goods['gid'], $Data['num'], $Data['data'], $User);
        $Data['data'] = $Goods['InputData'];

        $Price = $Goods['price'] * $Data['num']; //售价
        $freight = 0; //运费加价金额
        if ($Goods['freight'] != -1) {
            $Fre = $DB->get('freight', '*', [
                'id' => $Goods['freight'],
            ...
            if ($Fre) {
                $freight = Price::Freight($Fre, $Data['data'], $Goods['price'], $Data['num']);
                $freight -= $Price;
            }
        }
        ...
        foreach (explode('|', $Goods['input']) as $key => $value) {
            if (strstr($value, '{') && strstr($value, '}')) {
                ...
                if ($InputArr[$key] && $InputArr[$key]['type'] == 3) {
                    $Multiple = (int)$InputArr[$key]['num']; //需要加价的倍数
                    $Num = ($Data['data'][$key] - 0) ?? 1; //用户输入的倍数
